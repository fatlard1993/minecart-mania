#!/usr/bin/env python3
"""Generate Minecart Mania's icon: a cart on a copper powered rail.
...
OUT = os.path.join(HERE, "src/main/resources/assets/minecart-mania-justfatlard/icon.png")
...
def build_icon():
    rail = vanilla("block/powered_rail_on.png")
    # Gold to copper, the same rule generate_textures.py applies
    table = {(201, 136, 29): (178, 98, 71), (220, 174, 41): (214, 123, 91)}
    sprite = [[(table.get(px[:3], px[:3]) + (px[3],)) if px[3] else px for px in row] for row in rail]
    stamp(sprite, vanilla("item/minecart.png"), 0, 0)
